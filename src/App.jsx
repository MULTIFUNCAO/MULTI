...
function AuthHeader({ isPro, notifCount, userRole, onAlerts, userLocation = "Sua localização" }) {
  ...
  return (
    <div style={{
      ...
      <button onClick={()=>{ const newRole=activeRole==="professional"?"client":"professional"; if(onToggleRole)onToggleRole(newRole); setRole(newRole); try{ const s=JSON.parse(localStorage.getItem("multiSession")||"{}")||{}; s.role=newRole; localStorage.setItem("multiSession",JSON.stringify(s)); localStorage.setItem("multiMode",newRole); }catch(e){} }} style={{marginTop:8,padding:"8px 16px",borderRadius:20,border:"none",cursor:"pointer",background:activeRole==="professional"?"rgba(255,255,255,.15)":"rgba(255,87,34,.8)",color:"white",fontWeight:700,fontSize:12}}>{role==="professional"?"Modo Cliente":"Modo Profissional"}</button>
    </div>
  );
}
      ...
      <div style={{ margin:"18px 16px 0", borderRadius:24, overflow:"hidden", position:"relative", boxShadow:"0 10px 32px rgba(0,0,0,.22)" }}>
        ...
        <div style={{ position:"relative", zIndex:1, padding:"22px 22px 18px" }}>
          ...
          <button
            ...
            {newOrder && <NewOrderCard order={newOrder} onAccept={()=>{stopNewOrderSound();setNewOrder(null);setOnline(false);onAcceptOrder&&onAcceptOrder({id:Date.now(),title:newOrder.category,category:newOrder.category,clientName:"Cliente",location:newOrder.location,value:newOrder.value,description:newOrder.description,photo:newOrder.photo,photos:newOrder.photos||[],status:"accepted",phase:1});}} onReject={()=>{stopNewOrderSound();setNewOrder(null);}} />}
            {/* radar icon */}

...
{/* Urgência */}
      ...
      <div>
        ...
        <button
            onClick={() => { if (canPublish) { supabase.from("pedidos").insert({cliente_id:safeGetUser().email||"anonimo",cliente_nome:safeGetUser().name||"Cliente",categoria:form.cat,descricao:form.desc,valor:Number(form.value),cep:form.cep,fotos:window._photos||[],status:"aberto"}).then(({error})=>{if(error)console.warn("supabase:",error)}); onSuccess({ cat:form.cat, desc:form.desc, value:Number(form.value), cep:form.cep, photos:window._photos||[],photo:(window._photos&&window._photos[0])||null, cepInfo, material:form.material }); }}}

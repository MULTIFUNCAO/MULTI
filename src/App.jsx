  ...
  return (
    <div style={{
      ...
      {!isProfessional && (
        ...
        <button onClick={function(){var nr=role==="professional"?"client":"professional";setRole(nr);try{var s=JSON.parse(localStorage.getItem("multiSession")||"{}")||{};s.role=nr;localStorage.setItem("multiSession",JSON.stringify(s));}catch(x){}}} style={{marginTop:6,padding:"6px 16px",borderRadius:16,border:"none",cursor:"pointer",background:role==="professional"?"rgba(255,255,255,.2)":"rgba(255,87,34,.85)",color:"white",fontWeight:700,fontSize:12}}>{role==="professional"?"Modo Cliente":"Modo Profissional"}</button>
      {isProfessional && (

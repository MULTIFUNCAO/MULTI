      ...
      <div style={{ minHeight:"100vh", background:"#F5F6FA", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ padding:"20px 16px", display:"flex", flexDirection:"column", gap:14, alignItems:"center" }}>
          <div style={{ background:"white", borderRadius:20, padding:20, width:"100%", textAlign:"center", boxShadow:"0 3px 16px rgba(0,0,0,.09)" }}>
            ...
            {/* QR Code placeholder */}
            <div style={{ width:200, height:200, margin:"0 auto 16px", borderRadius:16, border:"3px solid #1a1a2e", background:"white", display:"grid", gridTemplateColumns:"repeat(9,1fr)", gap:2, padding:10, boxSizing:"border-box" }}>
              {Array.from({length:81}).map((_,i) => <div key={i} style={{ background:(i%3===0||i%7===1||i<9||i>=72||i%9===0||i%9===8)?"#1a1a2e":"white", borderRadius:1 }} />)}
            </div>
  ...
  const renderContent = () => {
    ...
    if (role === "client") {
      ...
      if (screen === "profile") {
        ...
        return <ProfileScreen role="client" isPro={false} userName={userName} onUpgrade={() => setScreen("upgrade")} onLogout={handleLogout} showToast={showToast} onOpenAdmin={() => setShowAdmin(true)} />;
      }
    ...
    if (screen === "profile") {
      if (!isLoggedIn) return <GuestProfileTab onLogin={() => setAuthScreen("welcome")} />;
      return <ProfileScreen role="professional" isPro={isPro} userName={userName} onUpgrade={() => setScreen("upgrade")} onLogout={handleLogout} showToast={showToast} onOpenWallet={() => setScreen("wallet")} onOpenAdmin={() => setShowAdmin(true)} docStatus={docStatus} onDocStatusChange={(id, st) => setDocStatus(d => ({ ...d, [id]: st }))} />;
    }

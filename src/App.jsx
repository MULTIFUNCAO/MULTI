...
function PostServiceScreen({ onBack, onSuccess }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18, padding:"18px 16px 40px" }}>
      ...
      <button onClick={() => { if (form.cat && form.desc && form.value) onSuccess({ cat:form.cat, desc:form.desc, value:Number(form.value) }); }} style={{ padding:"15px 0", borderRadius:14, border:"none", cursor:"pointer", background:`linear-gradient(135deg,${O},#E64A19)`, color:"white", fontWeight:900, fontSize:14, display:"flex", alignItems:"center", justifyContent:"center", gap:8, boxShadow:"0 5px 18px rgba(255,87,34,.30)" }}>
...
function ProUpgrade({ onBack, onSubscribe }) {
  ...
  const [seconds, setSeconds] = useState(300);     // 5-min PIX countdown
  const [copied,  setCopied]  = useState(false);

  const plans = [
    { id:"monthly",   label:"Mensal",     price:"29,90", period:"/mês",     badge:null,           pix:"29.90"  },
    { id:"quarterly", label:"Trimestral", price:"79,90", period:"/3 meses", badge:"Economize 11%", pix:"79.90"  },
    { id:"annual",    label:"Anual",      price:"249,90",period:"/ano",     badge:"🏆 Melhor valor!", pix:"249.90" },
  ];
  const chosen = plans.find(p => p.id === sel);

  // Fake PIX code (realistic format)
  const pixCode = `00020126580014br.gov.bcb.pix0136multi-funcao-${Date.now().toString(36).toUpperCase()}5204000053039865406${chosen?.pix.replace(".","").padStart(10,"0")}5802BR5913Multi Funcao6009Sao Paulo62070503***6304`;

  // Countdown timer while on PIX screen
  useEffect(() => {
    if (step !== "pix") return;
    if (seconds <= 0) return;
  ...
  // Simulate payment confirmation after copy (demo mode)
  const handleConfirmPayment = () => {
    setStep("done");
    setTimeout(() => { onSubscribe(); }, 2800);
  };
          ...
          {expired ? (
            ...
            <>
              ...
              <div style={{ background:"white", borderRadius:20, padding:20, textAlign:"center", boxShadow:"0 3px 16px rgba(0,0,0,.09)" }}>
                ...
                {/* QR placeholder — real app would use a QR library */}
                <div style={{ width:180, height:180, margin:"0 auto 14px", borderRadius:16, background:"white", border:"3px solid #1a1a2e", display:"grid", gridTemplateColumns:"repeat(9,1fr)", gap:2, padding:10, boxSizing:"border-box" }}>
      ...
      <div style={{ flex:1, padding:"18px 16px 40px", display:"flex", flexDirection:"column", gap:14, overflowY:"auto" }}>
        ...
        <div style={{ background:"white", borderRadius:20, overflow:"hidden", boxShadow:"0 4px 18px rgba(0,122,255,.12)", border:`2px solid ${B}` }}>
          ...
          <div style={{ padding:"18px 16px" }}>
            ...
            <button
              onClick={() => { setSeconds(300); setStep("pix"); }}
              style={{ width:"100%", padding:"15px 0", borderRadius:14, border:"none", cursor:"pointer", background:`linear-gradient(135deg,${B},#0055d4)`, color:"white", fontWeight:900, fontSize:15, display:"flex", alignItems:"center", justifyContent:"center", gap:10, boxShadow:`0 5px 18px ${B}44`, marginBottom:10 }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round">
                <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/>
                <path d="M14 14h3v3M17 17v3M14 20h3"/>
              </svg>
              Gerar QR Code
            </button>
...
function AdminAccessTrigger({ onOpenAdmin }) {
  ...
  const tap = () => { const n = taps + 1; setTaps(n); if (n >= 5) setTaps(0); };
  return (
    ...
    <div style={{ display:"flex", flexDirection:"column", paddingBottom:40 }}>
      ...
      {role === "client" && (
        <>
          ...
          {/* Addresses */}
          <SectionLabel label="Meus Endereços" />
          <div style={{ background:"white" }}>
            {[
              { icon:"🏠", label:"Minha Casa",        sub:"Rua das Flores, 123 — sua região" },
              { icon:"🏢", label:"Apartamento Centro", sub:"Av. Paulista, 456 — São Paulo" },
            ].map((addr, i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:13, padding:"13px 16px", borderBottom:"1px solid #F8F8F8" }}>
                <span style={{ width:36, height:36, borderRadius:11, background:B+"12", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>{addr.icon}</span>
                <div style={{ flex:1 }}>
                  <p style={{ fontSize:13, fontWeight:800, color:"#1a1a2e", marginBottom:1 }}>{addr.label}</p>
                  <p style={{ fontSize:11, color:"#bbb" }}>{addr.sub}</p>
                </div>
                <ChevronRight size={15} color="#DDD" />
              </div>
            ))}
            <button style={{ width:"100%", padding:"12px 0", border:"none", background:"none", display:"flex", alignItems:"center", justifyContent:"center", gap:7, color:B, fontWeight:800, fontSize:13, cursor:"pointer" }}>
              <Plus size={14} /> Adicionar endereço
            </button>
          </div>

          {/* Payment cards */}
          <SectionLabel label="Cartões de Pagamento" />
          <div style={{ background:"white" }}>
            {[
              { flag:"💳", label:"Nubank •••• 4821", sub:"Crédito — Principal" },
              { flag:"💳", label:"Bradesco •••• 7703", sub:"Débito" },
            ].map((card, i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:13, padding:"13px 16px", borderBottom:"1px solid #F8F8F8" }}>
                <span style={{ width:36, height:36, borderRadius:11, background:"#F3E5F5", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>{card.flag}</span>
                <div style={{ flex:1 }}>
                  <p style={{ fontSize:13, fontWeight:800, color:"#1a1a2e", marginBottom:1 }}>{card.label}</p>
                  <p style={{ fontSize:11, color:"#bbb" }}>{card.sub}</p>
                </div>
                <ChevronRight size={15} color="#DDD" />
              </div>
            ))}
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes typing-dot { 0%,80%,100%{transform:scale(0);opacity:.3} 40%{transform:scale(1);opacity:1} }
        @keyframes deal-in { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)} }
        .pulse-green { animation: pulse-dot 1.8s ease-in-out infinite; }
        .tdot { display:inline-block; width:6px; height:6px; border-radius:50%; background:#aaa; animation: typing-dot 1.2s infinite ease-in-out; }
        .tdot:nth-child(1){animation-delay:0s} .tdot:nth-child(2){animation-delay:.2s} .tdot:nth-child(3){animation-delay:.4s}
        .deal-card { animation: deal-in .3s ease-out; }
      `}</style>
                    ...
                    {!accepted ? (
                      ...
                      <div style={{ padding:"0 16px 16px", display:"flex", flexDirection:"column", gap:8 }}>
                        ...
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                          ...
                          <button style={{ padding:"11px 0", borderRadius:12, border:"none", background:`linear-gradient(135deg,${G},#16a34a)`, color:"white", fontWeight:800, fontSize:11, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:5 }}>
...
/* mini helper rendered inside modal */
function Section({ title, children }) {
  return (
    <div style={{ marginBottom:16 }}>
      <p style={{ fontSize:12, fontWeight:900, color:"#1a1a2e", margin:"0 0 5px" }}>{title}</p>
      <p style={{ fontSize:12, color:"#555", lineHeight:1.7, margin:0 }}>{children}</p>
    </div>
  );
}
  ...
  if (step === "success") {
    return (
      <div style={{ minHeight:"100vh", background:"#F8F9FA", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:32, textAlign:"center" }}>
        ...
        <button
          onClick={() => onComplete(name, email.trim(), true, cepFound ? "Sua cidade" : "sua região", role)}
...
function AdminDashboard({ onExit }) {
  ...
  const Card = ({ children, style = {} }) => (
    <div style={{ background:D1, borderRadius:18, border:`1px solid ${D2}`, padding:18, ...style }}>
      {children}
    </div>
  );
...
export default function App() {
  ...
  const [userLocation,  setUserLocation]  = useState("sua região");
  ...
  const handleLoginComplete = (name = "", email = "", isNewAccount = false, location = "", registeredRole = "") => {
    setIsLoggedIn(true);
    setAuthScreen(null);
    ...
    // If registration specified a role, apply it

...
{/* Urgência */}
      ...
      <div>
        ...
        <button
            onClick={() => { if (canPublish) { (async()=>{const urls=await Promise.all((window._photos||[]).map(async(b64,i)=>{const res=await fetch(b64);const blob=await res.blob();const ext=blob.type.includes("png")?"png":"jpg";const path="pedido_"+Date.now()+"_"+i+"."+ext;const{error:ue}=await supabase.storage.from("pedidos-fotos").upload(path,blob,{contentType:blob.type,upsert:true});if(ue){console.warn("upload:",ue);return null;}return supabase.storage.from("pedidos-fotos").getPublicUrl(path).data.publicUrl;}));const fotos=urls.filter(Boolean);supabase.from("pedidos").insert({cliente_id:safeGetUser().email||"anonimo",cliente_nome:safeGetUser().name||"Cliente",categoria:form.cat,descricao:form.desc,valor:Number(form.value),cep:form.cep,fotos,status:"aberto"}).then(({error})=>{if(error)console.warn("supabase:",error);});})(); onSuccess({ cat:form.cat, desc:form.desc, value:Number(form.value), cep:form.cep, photos:window._photos||[],photo:(window._photos&&window._photos[0])||null, cepInfo, material:form.material }); }}}

      ...
      <div style={{ margin:"18px 16px 0", borderRadius:24, overflow:"hidden", position:"relative", boxShadow:"0 10px 32px rgba(0,0,0,.22)" }}>
        ...
        <div style={{ position:"relative", zIndex:1, padding:"22px 22px 18px" }}>
          ...
          <button
            ...
            {newOrder && <NewOrderCard order={newOrder} onAccept={()=>{stopNewOrderSound();setNewOrder(null);showToast?.("🎉 Pedido aceito! Vá até o cliente.", "G");setOnline(false);setActiveService({id:Date.now(),title:newOrder.category,category:newOrder.category,clientName:"Cliente",location:newOrder.location,value:newOrder.value,status:"accepted",phase:1});}} onReject={()=>{stopNewOrderSound();setNewOrder(null);}} />}
            {/* radar icon */}

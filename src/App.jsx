...
{/* Urgência */}
      ...
      <div>
        ...
        <button
            onClick={() => { if (canPublish) { (async()=>{ const ts=Date.now(); const {data:pedidoData,error:pedidoErr}=await supabase.from("pedidos").insert({cliente_id:safeGetUser().email||"anonimo",cliente_nome:safeGetUser().name||"Cliente",categoria:form.cat,descricao:form.desc,valor:Number(form.value),cep:form.cep,fotos:[],status:"aberto"}).select("id").single(); if(pedidoErr){console.warn("insert:",pedidoErr);return;} const pedidoId=pedidoData.id; const urls=await Promise.all((window._photos||[]).map(async(b64,i)=>{ const res=await fetch(b64); const blob=await res.blob(); const ext=blob.type.includes("png")?"png":"jpg"; const path="pedido_"+ts+"_"+i+"."+ext; const{error:ue}=await supabase.storage.from("pedidos-fotos").upload(path,blob,{contentType:blob.type,upsert:true}); if(ue){console.warn("upload:",ue);return null;} return supabase.storage.from("pedidos-fotos").getPublicUrl(path).data.publicUrl; })); const fotos=urls.filter(Boolean); await supabase.from("pedidos").update({fotos}).eq("id",pedidoId); onSuccess({cat:form.cat,desc:form.desc,value:Number(form.value),cep:form.cep,photos:fotos,photo:fotos[0]||null,cepInfo,material:form.material}); })(); }}}
...
function ProfessionalHome({ userName, isPro, feedServices, onViewService, onUpgrade, userLocation = "sua região", allDocsVerified, docStatus, onGoToDocs, onGoToOrders, onGoToWallet, onAcceptOrder, pendingOrders=[] }) {
  ...
  supabase.channel("pedidos_novos").on("postgres_changes",{event:"INSERT",schema:"public",table:"pedidos"},(payload)=>{
    const p=payload.new;console.log("PAYLOAD.NEW FOTOS:",typeof p.fotos,JSON.stringify(p.fotos));setNewOrder({category:p.categoria,location:p.cidade||"Guarulhos, SP",value:String(p.valor||"0"),description:p.descricao||"",photos:(()=>{try{const f=p.fotos;return Array.isArray(f)?f:(typeof f==="string"?JSON.parse(f):[]);}catch(e){return [];}})(),photo:(()=>{try{const f=p.fotos;const arr=Array.isArray(f)?f:(typeof f==="string"?JSON.parse(f):[]);return arr[0]||null;}catch(e){return null;}})()});
  }).subscribe();
